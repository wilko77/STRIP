...
class STRIPCompiler(RouterCompiler):
    """Base Router compiler"""
    subID = 0
    ...
    def interfaces(self, node):
        ...
        super(STRIPCompiler, self).interfaces(node)

        i = len(node.interfaces)
        addr, net = self.nextNet()
        node.interfaces.append(
            id = 'eth%d' % i,
    ...
    def nextNet(self):
        addr = netaddr.IPAddress('10.0.%d.1' % self.subID)
        net = netaddr.IPNetwork('10.0.%d.0/24' % self.subID)
        self.subID += 1
        return addr, net 
